 ...
 ************************************************************************/
import coboldataclasses.Data110172009;
import coboldataclasses.TempElement10172009;
import com.res.java.lib.*;
import java.util.Random;

public class BubbleSort10172009 extends Program {

	//*>=======================
	//@CobolSourceFile("Bubble-sort.cob",6,8) 
	//01  DATA1.
	private Data110172009 data1 = new Data110172009();
	...
	//01  TEMP-ELEMENT.
	private TempElement10172009 tempElement = new TempElement10172009();
	public static void main(String[] args) {
            long startClock=System.currentTimeMillis();
		BubbleSort10172009 instance_ = new BubbleSort10172009();
		instance_.doCobolGotoStart();
                System.out.println("Time taken: "+(System.currentTimeMillis()-startClock)+"ms");
		System.exit(0);
	}
	...
	Paragraph inputElements=new Paragraph(this) {
		...
		for(int i0=0;i0<getCount1();++i0) {
			...
			//Accept ELEMENT1 of data1(temp1)
                    if(i0==0)
			data1.setElement1(new Random(10240).nextLong(),(int)getTemp1());
                    else
			data1.setElement1(new Random(data1.getElement1(i0)).nextLong(),(int)getTemp1());
			...
			data1.setElement3(Data110172009.element3Fmt_.format(" "),(int)getTemp1());
		...
		for(int i1=0;i1<getCount1();++i1) {
			...
			//    with no advancing			
			System.out.print("("+Data110172009.element1Fmt_.format(data1.getElement1((int)getTemp1())));
			//@CobolSourceFile("Bubble-sort.cob",52,20) 
			//Display ","  ELEMENT2 of data1 (temp1)			
			//    with no advancing			
			System.out.print(","+Data110172009.element2Fmt_.format(data1.getElement2((int)getTemp1())));
			//@CobolSourceFile("Bubble-sort.cob",54,20) 
			//Display ","  ELEMENT3 of data1 (temp1)			
			//    with no advancing			
			System.out.print(","+Data110172009.element3Fmt_.format(data1.getElement3((int)getTemp1())));
	...
	Paragraph sortElements=new Paragraph(this) {
		...
		//...end-perform
			int maxTestTime=0;long maxMoveTime=0;
			long start=System.currentTimeMillis();long move=0;int movecount=0;long testTime=0;int testCount=0;
		for(setTemp1(1);getTemp1() <= getCount1();setTemp1(getTemp1()+1)) {
			...
			for(setTemp2(getTemp3());getTemp2() <= getCount1();setTemp2(getTemp2()+1)) {
				...
				boolean test=data1.getElement1((int)getTemp2()) < data1.getElement1((int)getTemp1());
				maxTestTime=Math.max(maxTestTime, (int)(System.currentTimeMillis()-testStart));
				testTime+=(System.currentTimeMillis()-testStart);testCount++;
				
				if(test) {
					long start1=System.currentTimeMillis();movecount++;
					...
					move+=System.currentTimeMillis()-start1;
					maxMoveTime=Math.max(maxMoveTime, (System.currentTimeMillis()-start1));
				}
			}
            if(getTemp1()%1000==0) {
                System.out.println(getTemp1()+"="+(System.currentTimeMillis()-start)+", Move="+movecount+"("+move+")"+
                		", Test="+testCount+"("+testTime+"), maxTest="+"("+maxTestTime+
                		"), maxMove="+"("+maxMoveTime+")");
                start=System.currentTimeMillis();
                move=0;movecount=0;
                testCount=0;testTime=0;
                }
		}
